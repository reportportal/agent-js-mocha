...
class ReportportalAgent extends Mocha.reporters.Base {
  ...
  getCurrentTestItemId() {
    ...
    const mostRecentTestId = activeTestIds[activeTestIds.length - 1];
    return mostRecentTestId || this.getCurrentSuiteId();
  }
  ...
  finishTest(test, autoStatus) {
    ...
    if (test.err) {
      finalDescription = (description || '').concat(
        `\n\`\`\`error\n${test.err.stack}\n\`\`\``,
      );
    }

    const finalStatus = status || autoStatus;
    const withoutIssue = finalStatus === testStatuses.SKIPPED && this.options.reporterOptions.skippedIssue === false;
  ...
  getHookStartTime(hook, hookRPType, parent) {
    ...
    if (hookRPType === entityType.BEFORE_METHOD) {
      ...
      if (mostRecentTest) {
        return Math.max(getBeforeHookStartTime(mostRecentTest.startTime), currentParentStartTime);
      }
    }
    ...
    if (hookRPType === entityType.BEFORE_SUITE) {
      ...
      return Math.max(getBeforeHookStartTime(hookStartTime), currentParentStartTime);
    }
