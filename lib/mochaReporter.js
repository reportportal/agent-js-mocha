...
class ReportPortalAgent extends Mocha.reporters.Base {
  ...
  onSuiteStart(suite) {
    // console.log(suite);
    if (!suite.root) {
      const isParentExists = this.options.parallel
        ? this.suites[suite.parent.__mocha_id__]
        : this.suitesInfo.has(suite.parent);

      if (isParentExists) {
        const parentId = this.getSuiteTempId(suite.parent);
        console.log(`Suite parent id: ${parentId}`);
  ...
  getSuiteTempId(suite) {
    if (this.options.parallel) {
      return this.suites[suite.id || suite.__mocha_id__].rpTempId;
    }
